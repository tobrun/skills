...
Refinements follow a strict authority order (recorded user intent beats the repo's reality beats settled decisions beats spec prose) and edit `spec.md` only; anything that would flip a decision, change user-visible scope, or add a dependency is never auto-applied - it lands as an escalation in `.dev/{plan-name}/spec-review_N.md` with the question it needs answered.
An APPROVED verdict hands the refined spec straight to `build`; escalations route through `scope`'s remediation mode first.
