...
- **HTML mockups** (UI-affecting work only): build 2-4 wildly different design directions as static HTML with fake data, cheap enough to throw away. The point is reacting to a real rendered page, not a description of one. Recommend `artifact-design` for construction quality; don't invoke it.
...
**How to run it**: use `AskUserQuestion`, one question per call, ordered so the question whose answer would most change the shape of the work comes first. After each answer, re-evaluate what's still open - an early answer can make a planned later question moot, or surface a new one. Stop once what's left is cosmetic (naming, copy, minor styling) rather than architectural.
