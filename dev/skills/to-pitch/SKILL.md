...
3. Capture the demo. For UI-affecting changes, launch the app via the already-installed `run` skill (invoked directly via the Skill tool, the same exception `implement` uses for its e2e pass), then record a short clip of the change working with `claude-in-chrome`'s `gif_creator`. For non-UI changes, skip the GIF and lead with the clearest before/after example instead.
